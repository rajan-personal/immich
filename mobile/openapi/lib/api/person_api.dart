...
class PersonApi {
  ...
  /// Performs an HTTP 'GET /person/album/{id}' operation and returns the [Response].
  /// Parameters:
  ///
  /// * [String] id (required):
  ///
  /// * [bool] withHidden:
  Future<Response> getAllPeopleFromAlbumWithHttpInfo(String id, { bool? withHidden, }) async {
    // ignore: prefer_const_declarations
    final path = r'/person/album/{id}'
      .replaceAll('{id}', id);
  ...
  Future<PeopleResponseDto?> getAllPeopleFromAlbum(String id, { bool? withHidden, }) async {
    final response = await getAllPeopleFromAlbumWithHttpInfo(id,  withHidden: withHidden, );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
  ...
  Future<FaceDto?> getFaceFromAsset(String id, { bool? withHidden, }) async {
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'FaceDto',) as FaceDto;
